...
public class Converter {
	...
	private String[] getArrayOrder(List<Order> listorder) {
		...
		for (Order order : listorder) {
			...
			string.append(",");
			string.append(order.getDate());
			string.append(",");
			string.append(order.getDateFinishOrder());
			string.append(",");
			string.append(order.getStage());
			...
			if (wordString[0].equals("2")) {
				...
				for (int j = 1; j < wordString.length; j++) {
					if (j == 1) {
						...
						order.setDate(wordString[j]);
					} else if (j == 7) {
						order.setDateFinishOrder(wordString[j]);
					} else {
						order.setStage(wordString[j]);
					}
				}
				listorder.add(order);
			}
