...
public class OrderDao implements IOrderDao {
	...
	public void addNew(IOrder t) {
		...
		try {
			...
			IBook book = t.getBook();
			statement = connection
					.prepareStatement("UPDATE `mydb`.`book` SET `idWriter`='" + book.getWriter().getIdWriter()
							+ "', `nameBook`='" + book.getNameBook() + "', `quantityBook`='" + book.getQuantityPages()
							+ "', `price`='" + book.getPrice() + "' WHERE `idBook`='" + book.getIdBook() + "'");
			statement.executeUpdate();
			connection.commit();
