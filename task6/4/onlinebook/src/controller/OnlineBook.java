...
public class OnlineBook {
	...
	private PropertiesOnlineBook propertiesOnlineBook = new PropertiesOnlineBook();
	...
	private List<Book> getReadFileBook() {
		...
		try (ObjectInputStream ois = new ObjectInputStream(
				new FileInputStream(propertiesOnlineBook.getRoatPropertyBook()))) {
	...
	private List<Order> getReadFileOrder() {
		...
		try (ObjectInputStream ois = new ObjectInputStream(
				new FileInputStream(propertiesOnlineBook.getRoatPropertyOrder()))) {
	...
	public void saveToDataBases() {
		try (ObjectOutputStream oos = new ObjectOutputStream(
				new FileOutputStream(propertiesOnlineBook.getRoatPropertyBook()))) {
			oos.writeObject(getListBookAll());
		} catch (Exception e) {
			log.error(e);
		}
		try (ObjectOutputStream oos = new ObjectOutputStream(
				new FileOutputStream(propertiesOnlineBook.getRoatPropertyOrder()))) {
	...
	public void exportBookCSV(String name) {

		Book book = bookService.searchBook(name);
		if (book != null) {
			importexport.exportBookCSV(book, propertiesOnlineBook.getRoatImportExportBook());
		}

	}

	public void exportOrderCSV(String name) {
		Order order = orderservice.searchOrder(name);
		if (order != null) {
			importexport.exportOrderCSV(order, propertiesOnlineBook.getRoatImportExportOrder());
		}

	}

	public void importBookCSV() {
		Book book = importexport.importBookCSV(propertiesOnlineBook.getRoatImportExportBook());
		if (book != null) {
	...
	public void importOrderCSV()  {
        Order order=importexport.importOrderCSV(propertiesOnlineBook.getRoatImportExportOrder());
        if(order!=null){
        	Order order2=orderservice.searchOrder(order.getLastname());
        	if(order2!=null){
        		orderservice.updateOrder(order);
        	}else{
        		try {
					orderservice.createNewOrder(order.getLastname(), order.getFirstname(), order.getBook().getName());
				} catch (ParseException e) {
					log.error(e);
				}
        	}
